            ...
            <v-card-item>
                ...
                >
                    <v-btn
                        ...
                        class="mr-2"
                        color="blue"
                        @click="modifyArticle(noticeArticle.boardNo)"
                    ></v-btn>
                    <v-btn
                        icon="mdi-delete"
                        class="mr-2"
...
const modifyArticle = (boardNo) => {
    router.push('/notice/modify/' + boardNo)
}
...
const confirmHandler = (event) => {
    if (event.modalType === 'DELETE') {
        // 삭제 처리 callback
        deleteArticle(noticeArticle.value.boardNo)
...
const deleteArticle = async (boardNo) => {
    const response = await noticeBoardApi.deleteArticle(boardNo);
